...
estimated_vector <- as.integer(c(1,2))
...
# # LOAD TRAN IDS
# split_id_len <- nrow(run_table_dt)/length(climate_vector)/3
...
keep_vars <- c("config", "config_path", "run_table_dt", "defaults", 
               "run_yaml_from_table", "set_default_ids_in_yaml",
               "remove_selected_variables_from_env", "modify_yaml_settings_vector",
               "modify_recursive")
