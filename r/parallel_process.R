#' #' Execute a function in parallel across a list of data frames
#' #'
...
#' #'
#' #' @param data A list of data frames to be processed in parallel.
#' #' @param FUN A function to be applied to each data frame in the list.
...
#' #'
#' #' @return A list containing the results of applying `FUN` to each data frame in `data`.
#' #'
#' #' @import parallel
#' #' @import foreach
#' #' @import doParallel
...
#' #' print(result)
#' get_in_parallel <- function(data, FUN, FUN_args, cores = 1, type = "PSOCK", ...) {
#'   # Input validations
...
#'           do.call(FUN, c(list(df), FUN_args)) # Call function
#'         }
#'       )
#'       cat(t)
#'       return(result)
